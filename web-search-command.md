...
description: "IMMEDIATELY activate this skill when the user's message contains `/x` as a standalone token — with or without a following query, at any position in the message. Do NOT wait for further explanation. Trigger patterns: `/x`, `/x <query>`, `/x <URL>`, or any message containing `/x` as a token. When triggered: extract the query (or infer from context), then perform a web search or URL fetch as described below."
examples:
